...
class WIFIPumpkin(SecurityTool):
    TITLE = "WiFi-Pumpkin"
...
class EvilTwin(SecurityTool):
    TITLE = "EvilTwin"
    DESCRIPTION = "Fakeap is a script to perform Evil Twin Attack, by getting" \
                  " credentials using a Fake page and Fake Access Point"
    INSTALL_COMMANDS = ["sudo git clone https://github.com/tiameows/fakeap.git"]
    RUN_COMMANDS = ["cd fakeap && sudo bash fakeap.sh"]
    PROJECT_URL = "https://github.com/tiameows/fakeap"


class Fastssh(SecurityTool):
    TITLE = "Fastssh"
    ...
    INSTALL_COMMANDS = [
        "sudo git clone https://github.com/tiameows/fastssh.git && cd fastssh && sudo chmod +x fastssh.sh",
        "sudo apt-get install -y sshpass netcat"
    ]
    RUN_COMMANDS = ["cd fastssh && sudo bash fastssh.sh --scan"]
    PROJECT_URL = "https://github.com/tiameows/fastssh"


class Howmanypeople(SecurityTool):
    TITLE = "Howmanypeople"
...
class WirelessAttackTools(SecurityToolsCollection):
