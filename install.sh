...
if [[ $choice =~ ^[1-2]+$ ]]; then
    ...
    if curl -s -m 10 https://www.google.com > /dev/null || curl -s -m 10 https://www.github.com > /dev/null; then
        ...
        echo "";
        echo -e "${YELLOW}[✔] Downloading security-toolkit...${NC}"
        if sudo git clone https://github.com/tiameows/security-toolkit.git $install_dir; then
...
#            echo '#!/bin/bash' > hackingtool.sh
            echo '#!/bin/bash' > $install_dir/security-toolkit.sh
            echo "source $install_dir/venv/bin/activate" >> $install_dir/security-toolkit.sh
            echo "python3 $install_dir/security-toolkit.py \$@" >> $install_dir/security-toolkit.sh
            chmod +x $install_dir/security-toolkit.sh
            sudo mv $install_dir/security-toolkit.sh $bin_dir/security-toolkit
            echo -e "${GREEN}[✔] Script created successfully [✔]"
        else
            echo -e "${RED}[✘] Failed to download Security Toolkit [✘]"
            exit 1
        fi
    ...
    if [ -d $install_dir ]; then
        ...
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]     ✔✔✔ Now Just Type In Terminal (security-toolkit) ✔✔✔      [+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
    else
